...
export default function OverviewSettings({data,action,busy}:{data:DashboardData;action:Action;busy:boolean}){
  ...
  const [editing,setEditing]=useState(false),[enabled,setEnabled]=useState(current.enabled),[sameWebhook,setSameWebhook]=useState(!current.hasWebhook),[webhook,setWebhook]=useState(''),[messageId,setMessageId]=useState('');
  ...
  async function save(event:React.FormEvent){event.preventDefault();if(await action({action:'notifications.save',channel:'overview',enabled,useIncidentWebhook:sameWebhook,webhook})){setWebhook('');setEditing(false);}}
  return <section className="section-block"><div className="channel-row overview-heading"><LayoutList size={22}/><div><h2>Discord overview</h2><p>One message, updated every minute.</p></div><button className="small-button" disabled={busy} aria-expanded={editing} onClick={()=>{setEnabled(current.enabled);setSameWebhook(!current.hasWebhook);setWebhook('');setEditing(!editing);}}>{editing?'Close':'Edit overview'}</button></div>
    {editing&&<form className="connection-form" onSubmit={save} aria-label="Overview settings"><fieldset disabled={busy}><legend className="sr-only">Overview connection</legend><label className="connection-enabled"><input type="checkbox" checked={enabled} onChange={e=>setEnabled(e.target.checked)}/>Enable overview</label><label className="connection-enabled"><input type="checkbox" checked={sameWebhook} onChange={e=>setSameWebhook(e.target.checked)}/>Use incident webhook</label>{!sameWebhook&&<label className="connection-field">Overview webhook URL<input type="password" autoComplete="new-password" value={webhook} onChange={e=>setWebhook(e.target.value)} maxLength={500} required={enabled&&!current.hasWebhook} placeholder={current.hasWebhook?'Saved webhook — leave blank to keep':'Paste Discord webhook URL'}/></label>}<div className="connection-actions"><button className="primary-button" type="submit">Save changes</button><button className="small-button" type="button" onClick={()=>{setEditing(false);setWebhook('');}}>Cancel</button></div></fieldset></form>}
    <p className="overview-sync-status">{!current.enabled?'Disabled':!ready?'Configure a Discord webhook to start.':data.overview?.last_updated_at?`Updated ${timestamp(data.overview.last_updated_at)}${current.hasWebhook?'':' · Uses incident webhook'}`:'Waiting for the first update.'}</p>

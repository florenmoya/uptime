...
function ConnectionForm({channel,settings,action,busy,close}:{channel:'discord'|'email';settings:NotificationSettingsView;action:Action;busy:boolean;close:()=>void}){
  ...
  return <form className="connection-form" onSubmit={save} aria-label={`${channel==='discord'?'Discord':'Email'} settings`}>
    <fieldset disabled={busy}><legend className="sr-only">{channel==='discord'?'Discord':'Email'} connection</legend>
      <label className="connection-enabled"><input type="checkbox" checked={enabled} onChange={e=>setEnabled(e.target.checked)}/>Enable {channel==='discord'?'Discord incident':'email'} notifications</label>
      {channel==='discord'?<label className="connection-field">Incident webhook URL<input type="password" autoComplete="new-password" spellCheck={false} value={webhook} onChange={e=>setWebhook(e.target.value)} required={enabled&&!settings.discord.hasWebhook} maxLength={500} placeholder={settings.discord.hasWebhook?'Saved webhook — leave blank to keep':'Paste Discord webhook URL'}/><small>Changing this keeps the overview in its current channel.</small></label>:<div className="connection-grid">
...
export default function NotificationConnections({settings,action,busy}:{settings:NotificationSettingsView;action:Action;busy:boolean}){
  ...
  return <div className="notification-connections">{(['discord','email'] as const).map(channel=>{
    ...
    const name=channel==='discord'?'Discord incidents':'Email';
    return <div key={channel}><div className="channel-row">{channel==='discord'?<MessageSquare size={22}/>:<Mail size={22}/>}<div><h3>{name} <span className={`connection-label ${configured&&enabled?'connected':''}`}>{!configured?'Not configured':enabled?'Enabled':'Disabled'}</span></h3>{channel==='email'&&configured&&<p>{settings.email.to.join(', ')}</p>}</div><div className="notification-channel-actions">{channel==='discord'&&<button type="button" role="switch" aria-checked={enabled} aria-label="Discord incident notifications" className={`switch ${enabled?'on':''}`} disabled={busy||(!enabled&&!configured)} onClick={async()=>{if(await action({action:'notifications.save',channel,enabled:!enabled}))setEditing(null);}}><span/></button>}<button type="button" className="small-button" disabled={busy} aria-expanded={editing===channel} onClick={()=>setEditing(editing===channel?null:channel)}>{editing===channel?'Close':channel==='discord'?'Edit webhook':'Edit email'}</button></div></div>{editing===channel&&<ConnectionForm key={channel} channel={channel} settings={settings} action={action} busy={busy} close={()=>setEditing(null)}/>}</div>;
  })}</div>;
}

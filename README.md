...
**Settings → Discord overview** maintains one live Discord message with all services, current status, response time, observed 24-hour check results and coverage, and six ten-minute history blocks that fit narrow Discord message cards. It refreshes every 60 seconds through the existing worker. It is enabled by default and uses the incident webhook unless a separate encrypted webhook is saved. It is independent of the automatic incident alert switch.
